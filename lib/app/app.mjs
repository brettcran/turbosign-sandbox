/* lib/app/app.js — clean build (ESM) */
/* ================================== */
/* PDF.js boot (core + worker), rendering, annotations, session, save */
/* ================================== */

/* ---------- PDF.js core & worker (ultra-robust) ---------- */
let pdfjsLib;
try {
  // Prefer your local copy if present:
...
// Verbosity to see useful errors (not too chatty)
try {
...
/** Boot a worker: local module → CDN classic workerSrc → CDN module */
async function bootPdfWorkerOnce(){
  ...
  try {
    ...
    if (await urlOk(localURL)) {
      try {
        const test = new Worker(localURL, { type:'module' }); test.terminate();
        pdfjsLib.GlobalWorkerOptions.workerPort = new Worker(localURL, { type:'module' });
        console.info('[PDF.js] Using local module worker:', localURL);
        return;
      } catch (e) {
        console.warn('[PDF.js] Local module worker failed, continue…', e);
      }
    }
  } catch (e) {
    console.warn('[PDF.js] Could not resolve local worker', e);
  }
  ...
  if (await urlOk(cdnClassic)) {
    ...
    pdfjsLib.GlobalWorkerOptions.workerSrc  = cdnClassic; // classic worker path
  ...
  if (await urlOk(cdnModule)) {
    try {
      const test = new Worker(cdnModule, { type:'module' }); test.terminate();
      pdfjsLib.GlobalWorkerOptions.workerPort = new Worker(cdnModule, { type:'module' });
      console.info('[PDF.js] Using CDN module worker:', cdnModule);
      return;
    } catch (e) {
      console.error('[PDF.js] CDN module worker failed', e);
    }
  }
...
class Zoom {
  ...
  _onWheel(e){
    ...
    e.preventDefault();
    this.setScale(this.scale*(e.deltaY<0?1.1:0.9), e.clientX, e.clientY, true);
  }
  _pd(e){
    if(this.suspended) return;
  ...
  _pm(e){
    ...
    if(this._pinching&&this._pts.size===2){
      ...
      if(this._lastDist){ this.setScale(this.scale*(d/this._lastDist), this._cx, this._cy, true); }
      this._lastDist=d;
    }
  }
  _pu(e){
...
class Annotations {
  ...
  _addSignature(overlay,x,y,dataURL,widthHint){
    ...
    img.onload=()=>{
      ...
      const r=maxW/(img.naturalWidth||maxW);
  ...
  _wireAnno(el, overlay, {resizable}){
    const startDrag=(e)=>{
      ...
      this._select(el); e.preventDefault();
    };
  ...
  _select(el){
    this.container.querySelectorAll('.anno').forEach(n=>n.classList.remove('selected'));
    this.selected=el; if(el) el.classList.add('selected');
    el?.classList.contains('text') && Settings.syncFromElement(el);
  }
...
class SigPad{
  ...
  dataURL(){ return this.canvas.toDataURL('image/png'); } // transparent background
}
...
async function renderPdfFromFile(file, container, scale=1){
  CURRENT_PDF.file = file; // keep original for save recovery
...
/* ---------- PDF byte validator for save self-heal ---------- */
    ...
    CURRENT_PDF.wraps.forEach((wrap, idx)=>{
      ...
      overlay.querySelectorAll('.anno').forEach(el=>{
        ...
        if (el.classList.contains('text')){
          ...
          // signature PNG with alpha preserved
