/* TurboSign — app.mjs (stable, gutter-aware pinch)
   - PDFs load 1:1 and centered
   - Pinch/zoom anchors under your fingers, compensates gutters (no drift)
   ...
   - Text annotations (double‑tap/click), Stamp ✓ (black), draggable/undo/redo
...
async function ensureWorkerV5Module(){
  // Prefer module worker (.mjs) in v5
  try {
    ...
    if (await headOK(localW)) {
      ...
      console.info('[PDF.js] Worker: local module', localW);
      return;
    }
  ...
  try {
    ...
    if (await headOK(cdnW)) {
      ...
      console.info('[PDF.js] Worker: CDN module', cdnW);
      return;
    }
  } catch(e){ console.warn('[PDF.js] CDN module worker not available', e); }

  // Last resort: main-thread (slower)
  pdfjsLib.GlobalWorkerOptions.workerPort = null;
  console.error('[PDF.js] No worker available — falling back to main thread rendering');
}
...
/* ---------- Zoom (gutter-aware; anchors to viewport) ---------- */
class Zoom {
  ...
  _bounds(scale=this.scale){
    const contentW=this.container.scrollWidth*scale;
    const contentH=this.container.scrollHeight*scale;
...
/* Prevent page-level pinch within our stage (iOS/Android) */
...
/* ---------- Center helpers (declared after zoom) ---------- */
...
class SigPad{
  ...
  dataURL(){ return this.canvas.toDataURL('image/png'); } // transparent ink on export
}
...
function wireUI(){
  ...
  on(refs.fileInput,'change',async e=>{
    ...
    // Offer restore if same filename
    try{
