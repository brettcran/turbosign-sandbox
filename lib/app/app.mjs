/* TurboSign 4.0 — stable ESM
   Patch: iOS Safari main‑thread rendering, PDF header validation, clearer errors.
   Everything else remains the same as your working 4.0 base.
*/
...
// --- Detect iOS Safari (WebKit) to avoid flaky module workers there
...
(function bootPdfjs(){
  try {
    if (isIOSWebKit) {
      // Force main‑thread on iOS Safari — most reliable
      pdfjsLib.GlobalWorkerOptions.workerPort = null;
      console.info('[PDF.js] iOS Safari detected → using main‑thread rendering');
    } else {
...
/* ---------- PDF validation (clearer errors) ---------- */
function isValidPdfBytes(bytes){
  if (!bytes || !bytes.length) return false;
  // Allow BOM or a few stray bytes before %PDF-
...
/* ---------- Centered, bounded zoom (unchanged) ---------- */
const zoom = {
...
/* ---------- Render with validation & clearer errors ---------- */
...
async function renderPdfFromData(bytes, container, scale=1){
  // Validate bytes up front for clearer feedback
  ...
  try {
    ...
    console.info(`[PDF.js] Document ready (${pdfjsLib.GlobalWorkerOptions.workerPort ? 'worker' : 'main-thread'})`);
  } catch(err){
