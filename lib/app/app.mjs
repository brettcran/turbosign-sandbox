...
class Zoom {
  ...
  fitToWidth(vpCssWidth){
    const stageW = this.scroll.clientWidth - 16;
    const base = Math.max(1, vpCssWidth || 800);
...
class Annotations {
  constructor(container){
    ...
    this.mode = null; this.history = []; this.redoStack = [];
  ...
  attachOverlays(wrapInfos){
    ...
    wrapInfos.forEach((info)=>{
      ...
      // iOS: prevent Safari smart-zoom on double-tap over overlays
      if (isIOS){
        on(ov,'dblclick',(e)=>{ e.preventDefault(); }, {passive:false});
      }

      // Touch: double‑tap to add (and prevent default so Safari won't zoom)
      if (isTouch){
        ...
        on(ov,'pointerdown',(e)=>{
          ...
          if (dt < 300 && dx < 24 && dy < 24){
            e.preventDefault();                    // 👈 stop smart-zoom
            this._addText(ov, x, y, true);
            Session.bumpDirty(); lastTap=0;
  ...
  _addSignature(overlay,x,y,dataURL,widthHint){
    ...
    img.onload = ()=>{
      const base = overlay.clientWidth;
      const desired = (isTouch ? 0.7 : 0.5) * base; // 👈 bigger on mobile
      const maxW = Math.min(desired, img.naturalWidth);
  ...
  serialize(){
    return this.overlays.map((ov)=> {
      ...
      ov.querySelectorAll('.anno').forEach(el=>{
        const r = el.getBoundingClientRect(), or = ov.getBoundingClientRect();
        const item = { kind: el.classList.contains('text') ? 'text' : el.classList.contains('stamp') ? 'stamp' : 'sign',
                       x: r.left - or.left, y: r.top - or.top, w: r.width, h: r.height };
        if (item.kind==='text'){
          const cs = getComputedStyle(el);
          item.text  = el.textContent || '';
          item.style = { size: parseFloat(cs.fontSize)||16, color: cs.color,
                         bold: (parseInt(cs.fontWeight,10) || 400) >= 600,
                         italic: cs.fontStyle === 'italic', family: cs.fontFamily };
...
class SigController {
  ...
  use(){
    ...
    for (const ov of ann.overlays){
      ...
      const area = vx * vy; if (area > bestArea){ bestArea = area; best = ov; }
    }
    const target = best || ann.overlays[0]; if (!target){ this.close(); return; }
    const rect = target.getBoundingClientRect();
    const x = rect.width * 0.5, y = rect.height * 0.5;
    this.ann._addSignature(target, x, y, this.sig.dataURL(), rect.width * (isTouch ? 0.7 : 0.5)); // bigger on mobile
...
async function renderPdfFromData(bytes, container, scale=1.5){
  ...
  let pdf; try { pdf = await pdfjsLib.getDocument({ data: bytes }).promise; } catch (err) { throw err; }
...
async function saveFlattened(){
  ...
  CURRENT_PDF.wraps.forEach((wrap, idx)=>{
    ...
    overlay.querySelectorAll('.anno').forEach(el=>{
      const r = el.getBoundingClientRect(), or = overlay.getBoundingClientRect();
      const fx = pageSize.w / vp.width, fy = pageSize.h / vp.height;
      const x = (r.left - or.left) * fx;
      const y = pageSize.h - ((r.top - or.top) + r.height) * fy;
      const w = Math.max(1, r.width * fx), h = Math.max(1, r.height * fy);
...
const Settings = (() => {
  ...
  function build(){
    ...
    sizeInp=$('#ts-size',panel); colorInp=$('#ts-color',panel); boldInp=$('#ts-bold',panel);
    italicInp=$('#ts-italic',panel); familySel=$('#ts-family',panel); familySel.value=state.family;
