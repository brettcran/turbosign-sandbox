/* TurboSign — robust ESM app.mjs (PDF open fixes)
   - PDF.js v5: prefer local /lib/build/*.mjs → fallback CDN
   - Worker boot with CORS-safe fallbacks (module worker → main thread)
   - Validates PDF bytes; detailed error toasts
   - Minimal annotator (text/stamp/sign), undo/redo, save with pdf-lib
*/
...
const toast = (msg, kind='ok', t=3200) => {
...
function isValidPdfBytes(bytes){
  ...
  // Accept leading BOM/whitespace up to a few bytes before %PDF-
  for (let i=0;i<8 && i+4<bytes.length;i++){
...
async function loadPdfJs(){
  ...
  // Worker strategy:
  // Try local module worker → CDN module worker → main thread
  try{
    const localWorker = new URL('../build/pdf.worker.mjs', import.meta.url).href;
    if (await headOK(localWorker)) {
      const t = new Worker(localWorker, { type:'module' }); t.terminate();
      pdfjsLib.GlobalWorkerOptions.workerPort = new Worker(localWorker, { type:'module' });
  ...
  try{
    const cdnWorker = CDN_BASE + 'pdf.worker.mjs';
    if (await headOK(cdnWorker)) {
      const t = new Worker(cdnWorker, { type:'module' }); t.terminate();
      pdfjsLib.GlobalWorkerOptions.workerPort = new Worker(cdnWorker, { type:'module' });
      console.info('[PDF.js] Worker: CDN module');
      return;
    }
  }catch(e){ /* ignore */ }

  // As a last resort, render on main thread (slower but works everywhere).
  pdfjsLib.GlobalWorkerOptions.workerPort = null;
  console.warn('[PDF.js] No worker available — using main thread');
}

// Load pdf-lib (pure ESM)
...
class Annotations{
  ...
  _addText(overlay,x,y,focus){
    const el=document.createElement('div'); el.className='anno text'; el.contentEditable='true'; el.style.left=`${x}px`; el.style.top=`${y}px`;
    ...
    overlay.appendChild(el); this._wireAnno(el, overlay, {resizable:false}); if(focus) this._focus(el); this._recordAdd(overlay, el);
  }
  _addStamp(overlay,x,y){ const el=document.createElement('div'); el.className='anno stamp'; el.style.left=`${x}px`; el.style.top=`${y}px`; el.innerHTML=`<svg viewBox="0 0 24 24"><path d="M4 12l4 4 12-12"/></svg>`; overlay.appendChild(el); this._wireAnno(el, overlay, {resizable:false}); this._recordAdd(overlay, el); }
  _addSignature(overlay,x,y,dataURL,widthHint){
    ...
    el.appendChild(img); overlay.appendChild(el); this._wireAnno(el, overlay, {resizable:true});
...
const sig = new SigPad($('#sig-pad'), $('#sign-modal'));
...
$('#sign-modal')?.classList.remove('show');
...
async function renderPdfFromData(bytes, container, scale=1){
  if (!isValidPdfBytes(bytes)) {
    toast('That file doesn’t look like a PDF.', 'err'); throw new Error('Invalid PDF header');
  }
  container.innerHTML=''; CURRENT_PDF.wraps=[]; CURRENT_PDF.vpCSSByPage=[];
  let pdf;
  try {
    pdf = await pdfjsLib.getDocument({ data: bytes, isEvalSupported: true }).promise;
  } catch (err) {
    console.error('[PDF.js] getDocument failed:', err);
    toast(`Could not open PDF (${err?.message||err})`, 'err', 4200);
    throw err;
  }
      ...
      overlay.querySelectorAll('.anno').forEach(el=>{
        ...
        if (el.classList.contains('text')){
          ...
          ops.push((async ()=>{
            ...
            }catch(err){ console.warn('Embed image failed:', err); }
          })());
        }
      });
    });
...
function wireUI(){
  ...
  on($('#sig-use'),'click',e=>{ e.stopPropagation(); sigCtl.use(); });
  on($('#sig-clear'),'click',e=>{ e.stopPropagation(); sig.clear(); });
  on($('#sig-cancel'),'click',e=>{ e.stopPropagation(); sigCtl.close(); });
}
wireUI();
