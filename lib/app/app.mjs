...
try {
  ...
  // hard fallback (rarely needed)
  pdfjsLib = await import('https://cdn.jsdelivr.net/npm/pdfjs-dist@5.4.54/build/pdf.mjs');
}

// Try module worker first; fall back to workerSrc (same file URL)
(() => {
  ...
  try {
    ...
    // Some engines still honor workerSrc with ESM file; if not, pdf.js will inline.
...
// ===== Save (local pdf-lib with CDN fallback) =====
async function saveFlattened(){
  if(!CURRENT_PDF.bytes){ toast('Open a PDF first','err'); return; }
  let PDFDocument,rgb,StandardFonts;
  try{ ({ PDFDocument, rgb, StandardFonts } = await import('../pdf-lib.min.js')); }
  catch{ ({ PDFDocument, rgb, StandardFonts } = await import('https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.min.js')); }
  ...
  try{
    ...
    CURRENT_PDF.wraps.forEach((wrap, idx)=>{
      ...
      overlay.querySelectorAll('.anno').forEach(el=>{
        ...
        if(el.classList.contains('text')){
          ...
          const rgbNums=(cs.color.match(/\d+/g)||[0,0,0]).map(n=>parseInt(n,10)/255);
          const font=(parseInt(cs.fontWeight,10)||400)>=600 ? helvB : helv;
          page.drawText(el.textContent||'', { x, y, size, font, color: rgb(...rgbNums) });
        } else if(el.classList.contains('stamp')){
          ...
          const p=new Promise(async (resolve)=>{
            try{
              ...
              if(img.src.startsWith('data:')){
                const b64=(img.src.split(',')[1]||''); const bin=atob(b64); bytes=new Uint8Array(bin.length); for(let i=0;i<bytes.length;i++) bytes[i]=bin.charCodeAt(i);
              } else { const res=await fetch(img.src); bytes=new Uint8Array(await res.arrayBuffer()); }
              const png=await pdf.embedPng(bytes);
              page.drawImage(png, { x, y, width:w*fx, height:h*fy }); resolve();
            }catch(err){ console.warn('PNG embed failed', err); resolve(); }
          });
          embedOps.push(p);
        }
      });
    ...
    toast('Downloaded ✔️');
  }catch(e){ console.error(e); toast('Could not save PDF','err'); }
}

// ===== Settings (unchanged robust version) =====
const Settings=(()=>{ /* same as prior robust panel (omitted here for brevity) */ 
  ...
  function build(){
    ...
    panel.innerHTML=`<div style="display:flex;gap:8px;align-items:center;margin-bottom:8px">
  ...
  function toggle(){ /* simple show/hide */ panel.style.display = panel.style.display==='none' ? 'block' : 'none'; }
  function syncFromElement(el){
    ...
    Object.assign(state,{ size:Math.max(16,parseFloat(cs.fontSize)||16), color:rgbToHex(cs.color)||'#000000', bold:(parseInt(cs.fontWeight,10)||400)>=600, italic:cs.fontStyle==='italic', family:cs.fontFamily });
    panel.querySelector('#ts-size').value=String(state.size); panel.querySelector('#ts-size-val').textContent=state.size; panel.querySelector('#ts-color').value=state.color; panel.querySelector('#ts-bold').checked=state.bold; panel.querySelector('#ts-italic').checked=state.italic; panel.querySelector('#ts-family').value=state.family;
    ann.textStyle={...state};
  }
