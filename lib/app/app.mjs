/* TurboSign — minimal stable ESM build
   - No external icon lib (inline SVG only)
   - PDF.js v5 (CDN ESM) + module worker
   - pdf-lib (ESM via +esm)
   - Working buttons & listeners, text/stamp/sign, undo/redo, save flatten
*/
...
try {
  // Use a module worker explicitly
...
class Annotations{
  ...
  /* clamp initial stamp placement inside overlay, keep immediate coordinates for stable drag */
  _addStamp(overlay,x,y){
    ...
    requestAnimationFrame(()=>{ // after layout, measure and clamp
  ...
  _onUp(e){
    ...
    if(changed) { /* mark dirty if you keep snapshots */ }
  }
