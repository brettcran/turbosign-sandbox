/* TurboSign 4.0 — stable ESM
   Patch: resilient PDF.js worker fallback (auto retry on main thread if worker fails)
   Also keeps: centered pinch, zoom caps, page jumper, annotations, save.
*/
...
// Try to use module worker; if anything fails later we'll drop to main-thread.
(function bootPdfjsWorker(){
  try {
    const w = new Worker(WORKER_URL, { type:'module' });
    pdfjsLib.GlobalWorkerOptions.workerPort = w;
    console.info('[PDF.js] Worker: module (CDN)');
  } catch (e) {
    console.warn('[PDF.js] Worker boot failed, using main thread', e);
  ...
  const onPM = (e)=>{ if(!pts.has(e.pointerId)) return; pts.set(e.pointerId,e);
    if(pinching && pts.size===2){
      ...
      if(lastDist){
        ...
        const factor = Math.pow(raw, 0.85); // damped
...
/* ---------- Render with worker fallback ---------- */
async function tryGetDocument(bytes){
  // First attempt with current worker setting
  try {
    const doc = await pdfjsLib.getDocument({ data: bytes }).promise;
    return { doc, usedWorker: pdfjsLib.GlobalWorkerOptions.workerPort !== null };
  } catch (err) {
    // If a worker is set, drop to main thread and retry once
    const hadWorker = pdfjsLib.GlobalWorkerOptions.workerPort !== null;
    if (hadWorker) {
      console.warn('[PDF.js] getDocument failed with worker; retrying on main thread', err);
      try {
        pdfjsLib.GlobalWorkerOptions.workerPort?.terminate?.();
      }catch{}
      pdfjsLib.GlobalWorkerOptions.workerPort = null;
      const doc2 = await pdfjsLib.getDocument({ data: bytes }).promise;
      return { doc: doc2, usedWorker: false };
    }
    throw err;
  }
}
...
async function renderPdfFromData(bytes, container, scale=1){
  ...
  let pdf;
  try {
    const { doc, usedWorker } = await tryGetDocument(bytes);
    pdf = doc;
    console.info(`[PDF.js] Document ready (${usedWorker ? 'worker' : 'main-thread'})`);
  } catch(err){
    console.error('PDF engine could not open the document', err);
...
function wireUI(){
  ...
  on(refs.fileInput,'change',async e=>{
    ...
    catch(err){ console.error(err); /* toast already shown in render */ }
  });
