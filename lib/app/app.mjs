// TurboSign v4.2 — ESM core with centered zoom, exclusive tools, text settings, insert photo
// ------------------------------------------------------------------------------------------
...
const refs = {
  ...
  tsSize: document.getElementById('ts-size'),
  tsSizeVal: document.getElementById('ts-size-val'),
  tsColor: document.getElementById('ts-color'),
  tsBold: document.getElementById('ts-bold'),
  tsItalic: document.getElementById('ts-italic'),
  tsFamily: document.getElementById('ts-family')
};
...
(function wirePinch(){
  ...
  // Prevent browser pinch inside stage (iOS gesture events)
...
/* ---------- Text Settings Panel ---------- */
function showTextPanel(show){
  refs.textPanel.style.display = show ? 'block' : 'none';
}
function applyTextPanel(part){
  Object.assign(textStyle, part||{});
  // live-apply to selected text (if any)
...
/* ---------- Toolbar (exclusive tools + icon swap already in index) ---------- */
function setActiveTool(tool){
  ...
  document.querySelectorAll('#toolbar .btn').forEach(b=>{
    if (['text','stamp','sign'].includes(b.dataset.act)){
      ...
      icon?.setAttribute('icon', `fluent:${base}-${on ? 'filled' : 'regular'}`);
    } else {
      b.classList.remove('active');
    }
  });
  if (tool==='text') showTextPanel(true);
  else showTextPanel(false);
}
...
refs.toolbar.addEventListener('click', e=>{
  ...
  if (act==='help'){ toast('Open a PDF → double‑tap to add Text/Stamp/Signature or insert Photo → Save to flatten.'); return; }
  if (act==='sign'){ sig.open(); return; }
  if (act==='settings'){ showTextPanel(refs.textPanel.style.display==='none'); return; }
  if (act==='photo'){ refs.photoInput.value=''; refs.photoInput.click(); return; }

  if (['text','stamp','sign'].includes(act)){
    setActiveTool(activeTool===act ? null : act); // exclusive toggle
...
refs.photoInput.addEventListener('change', async e=>{
  ...
  reader.onload = ()=>{
    ...
    // place roughly center of first visible page
    const r = firstOverlay.getBoundingClientRect();
    const x = (r.width/2)/zoom.scale - 80, y = (r.height/2)/zoom.scale - 50;
...
document.addEventListener('DOMContentLoaded', ()=>{
  // nothing else needed; toolbar hover/touch swap is in index.html
});