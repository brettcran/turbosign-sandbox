// TurboSign — center-anchored pinch zoom + slide-in text drawer + fixed stamp + ESM everything
...
const refs = {
  ...
  // text drawer (auto shows on Text tool; hides after idle)
  drawer: $('#text-drawer'),
...
function addText(overlay, x, y){
  ...
  // Focus without page scroll; place caret at end
...
/* ---------- Overlays: dblclick & iOS double‑tap (stamp fixed) ---------- */
function armOverlays(){
  // reset any old handlers
  refs.container.querySelectorAll('.overlay').forEach(ov=> ov.replaceWith(ov.cloneNode(true)));
  refs.container.querySelectorAll('.overlay').forEach(ov=>{
    ov.addEventListener('dblclick', e=>{
      ...
      else if (activeTool==='stamp') addStamp(ov,x,y);            // ✅ fixed
      else if (activeTool==='sign' && signatureDataURL) addImageLike(ov,x,y,signatureDataURL,'sign');
    });

    // iOS double‑tap with preventDefault (no page zoom)
...
/* ---------- Text Drawer (auto show/hide) ---------- */
let drawerTimer=null;
function showDrawer(){
  refs.drawer.classList.add('show');
  restartDrawerAutoHide();
}
function hideDrawer(){ refs.drawer.classList.remove('show'); clearTimeout(drawerTimer); drawerTimer=null; }
function restartDrawerAutoHide(){
  clearTimeout(drawerTimer);
  drawerTimer = setTimeout(()=> hideDrawer(), 5000);
}
// interactions inside drawer reset timer
['input','change','pointerdown','pointerup'].forEach(ev=>{
  refs.drawer.addEventListener(ev, ()=> restartDrawerAutoHide(), {passive:true});
});

// bind controls to text style
refs.tsSize.addEventListener('input', ()=>{ refs.tsSizeVal.textContent=refs.tsSize.value; textStyle.size=+refs.tsSize.value; applyTextStyleToFocused(); }, {passive:true});
refs.tsColor.addEventListener('input', ()=>{ textStyle.color=refs.tsColor.value; applyTextStyleToFocused(); }, {passive:true});
refs.tsBold.addEventListener('change', ()=>{ textStyle.bold=!!refs.tsBold.checked; applyTextStyleToFocused(); });
refs.tsItalic.addEventListener('change', ()=>{ textStyle.italic=!!refs.tsItalic.checked; applyTextStyleToFocused(); });
refs.tsFamily.addEventListener('change', ()=>{ textStyle.family=refs.tsFamily.value; applyTextStyleToFocused(); });
function applyTextStyleToFocused(){
...
function setActiveTool(tool){
  ...
  if (tool==='text'){ // show drawer automatically
    showDrawer();
  } else {
    hideDrawer();
  }
...
/* ---------- Overlay wiring (dblclick/tap) ---------- */
function armOverlays(){
  // Purge old overlays and reattach handlers
  refs.container.querySelectorAll('.overlay').forEach(ov=> ov.replaceWith(ov.cloneNode(true)));
  refs.container.querySelectorAll('.overlay').forEach(ov=>{
    // Desktop dblclick
    ov.addEventListener('dblclick', e=>{
...
/* ---------- File picker trigger (Open) ---------- */
document.addEventListener('click', (e)=>{
  const btn = e.target.closest?.('[data-act="open"]'); if (!btn) return;
  e.preventDefault(); refs.fileInput.click();
});