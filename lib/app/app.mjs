...
try {
  ...
  // Fallback for browsers without module workers
...
// === Zoom (pinch + Ctrl/⌘ + wheel). Start at 1:1 (no auto fit) ===
class Zoom {
...
class Annotations {
  ...
  // --- coordinate helpers (UN‑SCALED CSS px) ---
  ...
  _wireAnno(el, overlay, {resizable}){
    const startDrag = (e)=>{
      ...
      zoom.suspend(true); // avoid pinch/gesture fighting during drag
    ...
    if (img) on(img, 'pointerdown', startDrag, {passive:false}); // allow drag starting on signature image
    ...
    if (resizable){
      ...
      on(h, 'pointerdown', (e)=>{
        ...
        $('#pdf-scroll').style.touchAction = 'none';
        zoom.suspend(true); // avoid zoom while resizing
...
class SigPad {
  ...
  clear(){
    ...
    c.clearRect(0,0,this.canvas.width,this.canvas.height); // transparent
...
function wireUI(){
  ...
  // Open handler (1:1 render; staged errors)
  on(refs.fileInput, 'change', async (e)=>{
    const file = e.target.files?.[0]; if (!file) return;
    let infos;
    try {
      infos = await renderPdfFromFile(file, refs.container, 1); // 1:1
    } catch (renderErr) {
...
// On startup: generic restore hint
