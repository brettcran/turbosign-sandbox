/* lib/app/app.mjs — signatures placed by double‑click/tap at cursor, plus full app */

/* ---------- PDF.js core & worker (ultra-robust) ---------- */
let pdfjsLib;
try {
  pdfjsLib = await import('../build/pdf.mjs'); // local build if present
} catch (e) {
  console.warn('[PDF.js] Local core not found, using CDN:', e);
...
async function bootPdfWorkerOnce(){
  if (bootPdfWorkerOnce._done) return;
  bootPdfWorkerOnce._done = true;

  // 1) Local module worker
  try{
    const localURL = new URL('../build/pdf.worker.mjs', import.meta.url).href;
    if (await urlOk(localURL)) {
      try { const t=new Worker(localURL,{type:'module'}); t.terminate();
        pdfjsLib.GlobalWorkerOptions.workerPort=new Worker(localURL,{type:'module'});
        console.info('[PDF.js] Using local module worker:', localURL); return;
      }catch(e){ console.warn('[PDF.js] Local module worker failed', e); }
    }
  }catch(e){ console.warn('[PDF.js] Could not resolve local worker', e); }

  // 2) CDN classic worker (most compatible)
  const cdnClassic='https://cdn.jsdelivr.net/npm/pdfjs-dist@5.4.54/build/pdf.worker.min.js';
  if (await urlOk(cdnClassic)){
    pdfjsLib.GlobalWorkerOptions.workerPort=undefined;
    pdfjsLib.GlobalWorkerOptions.workerSrc=cdnClassic;
    console.info('[PDF.js] Using CDN classic workerSrc:', cdnClassic);
    return;
  }

  // 3) CDN module worker
  const cdnModule='https://cdn.jsdelivr.net/npm/pdfjs-dist@5.4.54/build/pdf.worker.mjs';
  if (await urlOk(cdnModule)){
    try{ const t=new Worker(cdnModule,{type:'module'}); t.terminate();
      pdfjsLib.GlobalWorkerOptions.workerPort=new Worker(cdnModule,{type:'module'});
      console.info('[PDF.js] Using CDN module worker:', cdnModule); return;
    }catch(e){ console.error('[PDF.js] CDN module worker failed', e); }
  }
  console.error('[PDF.js] No worker could be started. Rendering may fail.');
}
await bootPdfWorkerOnce();
...
class Zoom {
  constructor(container, scroll){
    this.container=container; this.scroll=scroll; this.scale=1; this.min=0.6; this.max=3; this.suspended=false;
...
class Annotations {
  constructor(container){
    ...
    this.mode=null; // 'text' | 'stamp' | 'sign'
  ...
  attachOverlays(wrapInfos){
    ...
    wrapInfos.forEach(({wrap})=>{
      ...
      // clear selection on background press
      on(ov,'pointerdown', e=>{ if(e.target===ov) this._select(null); }, {passive:true});

      // common dblclick handler: routes by current mode
      on(ov,'dblclick', e=>{
        ...
        if(this.mode==='text'){
          ...
          if(!sigCtl.hasInk()){ sigCtl.open(); toast('Draw your signature, then press Use', 'ok', 2600); return; }
          const el=this._addSignature(ov,x,y,sigCtl.dataURL(), ov.clientWidth*0.5);
          this._select(el); scrollAnnoIntoView(el); Session.bumpDirty?.();
        }
      });

      // touch double‑tap for text/sign modes
      if(isTouch){
        let t=0,lx=0,ly=0;
        on(ov,'pointerdown',e=>{
...
class SigPad{
  ...
  dataURL(){ return this.canvas.toDataURL('image/png'); } // transparent pixels, white UI is CSS only
}
const sig = new SigPad(refs.sigPad, refs.sigModal);

class SigController{
  constructor(sig, ann){ this.sig=sig; this.ann=ann; this._ink=null; }
  open(){ this.sig.open(); }
  close(){ this.sig.close(); }
  hasInk(){ return !!this._ink; }
  dataURL(){ return this._ink; }
  arm(){ this.ann.setMode('sign'); toast(isTouch?'Signature: double‑tap to place':'Signature: double‑click to place'); }
  use(){ this._ink=this.sig.dataURL(); this.close(); this.arm(); } // store ink, keep in sign mode
  clearInk(){ this._ink=null; }
}
...
async function renderPdfFromData(bytes, container, scale=1){
  ...
  catch(err){ console.error('getDocument failed', err); toast('Could not start PDF engine','err'); throw err; }
...
async function saveFlattened(){
  ...
  try{
    ...
    a.href=URL.createObjectURL(blob); a.download=defaultName; document.body.appendChild(a); a.click(); a.remove();
    setTimeout(()=>URL.revokeObjectURL(a.href), 1200); toast('Downloaded ✔️');
...
function wireUI(){
  ...
  on(refs.fileInput,'change',async e=>{
    ...
    catch(err){ const msg=String(err?.message||err);
      if(msg.includes('Worker')||msg.includes('loading')) toast('PDF engine failed to start. Check worker path.','err',4200);
      else toast('Could not open PDF','err',3200); console.error(err); return; }
  ...
  // Signature button: if we have ink, arm placement; otherwise open pad
  on(refs.signBtn,'click',()=>{
    if(sigCtl.hasInk()){ ann.setMode(ann.mode==='sign'?null:'sign'); toast(ann.mode?(isTouch?'Signature: double‑tap to place':'Signature: double‑click to place'):'Tool off'); }
    else { ann.setMode(null); sigCtl.open(); }
  });

  on(refs.undoBtn,'click',()=>ann.undo());
  on(refs.redoBtn,'click',()=>ann.redo());
  on(refs.helpBtn,'click',()=>toast('📂 Open a PDF. Text: double‑tap/click; Stamp: tap; Signature: draw → Use, then double‑click/tap to place; Save to flatten.','ok',4200));
  ...
  on(refs.sigClear,'click',e=>{ e.stopPropagation(); sig.clear(); sigCtl.clearInk(); });
