/* TurboSign — app.mjs (stable, mobile pinch fixed)
   - PDFs load 1:1 and centered
   - Pinch/zoom anchors under fingers (no drift), clamps to bounds
   - Signature button always opens modal; double‑tap/click places signature at tap
   - Text tool (double‑tap/click), Stamp ✓ (black), draggable annos
   - Save with pdf-lib (flatten)
   - Session restore + toasts
   - Robust PDF.js boot (local /lib/build + CDN fallback)
*/

/* ---------- PDF.js core & worker (robust boot) ---------- */
let pdfjsLib;
...
/* ---------- Zoom (anchored under fingers; clamps) ---------- */
class Zoom {
  constructor(container, scroll){
    ...
    this._lastDist = 0;
    this._cx = 0; this._cy = 0; // pinch midpoint (client coords)
  ...
  setScale(newScale, cx, cy, keepCenter=true){
    ...
    // Anchor against the SCROLL viewport (not the container)
  ...
  _onWheel(e){
    ...
    if (!(e.ctrlKey || e.metaKey)) return; // ctrl/cmd + wheel only
    e.preventDefault();
  ...
  _pm(e){
    ...
    if (this._pinching && this._pts.size === 2){
      ...
      // Track live midpoint so the content stays under your fingers
      this._cx = (a.clientX + b.clientX)/2;
      this._cy = (a.clientY + b.clientY)/2;
...
/* Stop page-level pinch zoom within our stage (iOS/Android) */
...
/* Utility used above (declared after ann uses zoom at runtime only) */
...
class SigPad{
  ...
  dataURL(){ return this.canvas.toDataURL('image/png'); } // transparent ink on export (no white box on PDF)
}
...
class SigController{
  ...
  use(){
    ...
    toast('Signature ready — double‑tap/click to place');
  }
}
...
async function renderPdfFromData(bytes, container, scale=1){
  ...
  let pdf;
  try {
    pdf = await pdfjsLib.getDocument({ data:
