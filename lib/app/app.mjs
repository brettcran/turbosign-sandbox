/* TurboSign — app.mjs (stable + worker + pinch anchored)
   - PDF.js v5: module worker (local /lib/build/pdf.worker.mjs with CDN fallback)
   - PDFs load 1:1 and centered
   - Pinch zoom anchors under fingers, clamps scroll bounds
   - Signature button always opens modal; double‑tap/click to place
   - Text (double‑tap/click), Stamp ✓ (black)
   - Save with pdf-lib (flatten), Session restore, Toasts
*/

/* ---------- PDF.js core & worker (v5 module worker) ---------- */
let pdfjsLib;
...
// v5 prefers module worker; classic *.js isn’t shipped in all builds.
async function ensureWorkerV5Module(){
  ...
  try {
    ...
    if (await headOK(localW)) {
      const w = new Worker(localW, { type: 'module' });
      w.terminate(); // probe ok
  ...
  try {
    ...
    if (await headOK(cdnW)) {
      const w = new Worker(cdnW, { type: 'module' });
      w.terminate();
  ...
  // 3) Last resort: main-thread rendering (slower, but works)
...
(() => { // one-time pulse style for new items
...
/* ---------- Centering helper (declare before use) ---------- */
...
/* ---------- Zoom (anchor to SCROLL viewport; clamp bounds) ---------- */
class Zoom {
  ...
  _centerIfSmaller(){
    const { contentW, viewW, contentH, viewH } = this._bounds();
  ...
  setScale(newScale, cx, cy, keepCenter=true){
    ...
    const rect = this.scroll.getBoundingClientRect(); // anchor to viewport
...
// Stop page-level pinch zoom inside our stage (iOS/Android)
...
class Annotations {
  ...
  _addSignature(overlay,x,y,dataURL,widthHint){
    ...
    this._wireAnno(el, overlay, {resizable:true}); // signatures are resizable
    img.onload=()=>{
      ...
      // clamp inside page after size known
...
/* Keep new items visible after placement */
...
class SigPad{
  ...
  dataURL(){ return this.canvas.toDataURL('image/png'); } // transparent ink; no white box on PDF
}
...
async function renderPdfFromData(bytes, container, scale=1){
  ...
  try {
    ...
    console.error('getDocument failed', err); toast('Could not start PDF engine', 'err'); throw err;
  }
