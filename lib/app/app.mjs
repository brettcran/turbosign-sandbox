/* TurboSign — app.mjs (spacer-in-flow + DOM clamping = centered pinch)
   - PDF.js v5 ESM + worker (local → CDN → main-thread fallback)
   - Spacer is in normal flow so scrollWidth/Height are accurate
   - Pinch zoom anchors at midpoint; clamps using DOM extents (no drift)
   - Double‑tap text/sign; stamp; drag; signature resize; save/flatten; session restore
*/
...
/* ---------- spacer (in normal flow) & zoom ---------- */
// Spacer participates in normal layout so scrollWidth/Height reflect scaled size.
...
refs.scroll.appendChild(spacer);

// True, unscaled content size
const BASE={ width:0, height:0 };

const zoom={
  scale:1, min:0.6, max:3, suspended:false,
  _syncSpacer(){
    spacer.style.width = `${Math.max(1,Math.floor(BASE.width * this.scale))}px`;
    spacer.style.height= `${Math.max(1,Math.floor(BASE.height* this.scale))}px`;
  },
  setScale(newScale,cx,cy){
    ...
    // Expand/collapse scrollable area to match the new scale
    this._syncSpacer();

    let newLeft=contentX*this.scale-(cx-rect.left)+gx1;
    let newTop =contentY*this.scale-(cy-rect.top )+gy1;

    // Clamp using the actual DOM extents (prevents drift on some mobile browsers)
    const maxX=Math.max(0, refs.scroll.scrollWidth  - refs.scroll.clientWidth);
    const maxY=Math.max(0, refs.scroll.scrollHeight - refs.scroll.clientHeight);
    scroll.scrollLeft=clamp(newLeft,0,maxX);
    scroll.scrollTop =clamp(newTop ,0,maxY);

    if(refs.scroll.scrollWidth  <= refs.scroll.clientWidth)  scroll.scrollLeft=0;
    if(refs.scroll.scrollHeight <= refs.scroll.clientHeight) scroll.scrollTop =0;
  }
};
...
(function wirePinch(){
  ...
  const onPM=e=>{
    ...
    if(pinching && pts.size===2){
      ...
      cx=(a.clientX+b.clientX)/2; cy=(a.clientY+b.clientY)/2;
      if(lastDist){ const factor=d/lastDist; zoom.setScale(zoom.scale*factor,cx,cy); }
  ...
  // Block iOS page pinch inside stage
...
function scrollAnnoIntoView(el){
  ...
  const maxX=Math.max(0, refs.scroll.scrollWidth  - refs.scroll.clientWidth);
  const maxY=Math.max(0, refs.scroll.scrollHeight - refs.scroll.clientHeight);
  const targetLeft=clamp(elCX-refs.scroll.clientWidth/2,0,maxX);
  const targetTop =clamp(elCY-refs.scroll.clientHeight/2,0,maxY);
...
async function renderPdfFromData(bytes,container,scale=1){
  ...
  for(let i=1;i<=pdf.numPages;i++){
    try{
      ...
      totalH+=vpCSS.height + 24; // ~24px vertical gap/margins between pages
    }catch(e){ console.warn('Page render warning:',e); }
  }

  // Capture base size and sync spacer
