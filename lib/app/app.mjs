/* TurboSign — app.mjs (final: stable base size + gutter-aware pinch)
   - PDFs load 1:1 and centered
   - Pinch/zoom anchors under fingers, compensates gutters (no drift)
   - PDF.js v5 worker boot (module worker; local -> CDN -> main-thread fallback)
   - Signature button ALWAYS opens modal; double‑tap/click to place
   - Text (double‑tap/click), Stamp ✓ (black), draggable, undo/redo
   - Save (flatten) via pdf-lib, session restore, toasts
*/

/* ---------- PDF.js core & worker (v5) ---------- */
let pdfjsLib;
...
async function ensureWorkerV5Module(){
  try{
    const localW = new URL('../build/pdf.worker.mjs', import.meta.url).href;
    if(await headOK(localW)){
      const probe=new Worker(localW,{type:'module'}); probe.terminate();
      pdfjsLib.GlobalWorkerOptions.workerPort=new Worker(localW,{type:'module'});
      console.info('[PDF.js] Worker: local module', localW); return;
    }
  }catch(e){ console.warn('[PDF.js] local module worker not available', e); }

  try{
    const cdnW = CDN_BASE + 'pdf.worker.mjs';
    if(await headOK(cdnW)){
      const probe=new Worker(cdnW,{type:'module'}); probe.terminate();
      pdfjsLib.GlobalWorkerOptions.workerPort=new Worker(cdnW,{type:'module'});
      console.info('[PDF.js] Worker: CDN module', cdnW); return;
    }
  }catch(e){ console.warn('[PDF.js] CDN module worker not available', e); }

  pdfjsLib.GlobalWorkerOptions.workerPort=null;
  console.error('[PDF.js] No worker available — main-thread rendering fallback');
}
try{ pdfjsLib.setVerbosity?.((pdfjsLib.VerbosityLevel||{}).errors ?? 1); }catch{}
await ensureWorkerV5Module();
...
/* ---------- Zoom (stable base size + gutter-aware) ---------- */
class Zoom{
  constructor(container,scroll){
    ...
    // Base content size (unscaled) captured after first render
    this.baseW=container.scrollWidth; this.baseH=container.scrollHeight;

    this._pts=new Map(); this._pinching=false; this._lastDist=0; this._cx=0; this._cy=0;
    container.style.transformOrigin='0 0';

    this._onWheel=this._onWheel.bind(this);
    this._pd=this._pd.bind(this); this._pm=this._pm.bind(this); this._pu=this._pu.bind(this);
  ...
  /** Call this after pages render to lock the base content size (unscaled). */
  setBaseSize(w,h){ this.baseW=w||this.container.scrollWidth; this.baseH=h||this.container.scrollHeight; }
  suspend(v){ this.suspended=!!v; }

  _bounds(scale=this.scale){
    const contentW=(this.baseW||this.container.scrollWidth)*scale;
    const contentH=(this.baseH||this.container.scrollHeight)*scale;
    const viewW=this.scroll.clientWidth, viewH=this.scroll.clientHeight;
    const maxX=Math.max(0,contentW-viewW), maxY=Math.max(0,contentH-viewH);
    const gutterX=Math.max(0,(viewW-contentW)/2), gutterY=Math.max(0,(viewH-contentH)/2);
    return {contentW,contentH,viewW,viewH,maxX,maxY,gutterX,gutterY};
  }
  _clampXY(x,y,scale=this.scale){
    const {maxX,maxY}=this._bounds(scale);
    return {x:Math.max(0,Math.min(maxX,x)), y:Math.max(0,Math.min(maxY,y))};
  }
  _centerIfSmaller(scale=this.scale){
    const {contentW,contentH,viewW,viewH}=this._bounds(scale);
  ...
  setScale(newScale,cx,cy,keepCenter=true){
    ...
    const {gutterX:gx0,gutterY:gy0}=this._bounds(this.scale);
    const contentX=(this.scroll.scrollLeft+(cx-rect.left)-gx0)/this.scale;
    const contentY=(this.scroll.scrollTop +(cy-rect.top )-gy0)/this.scale;

    this.scale=newScale;
    this.container.style.transform=`scale(${this.scale})`;

    if(keepCenter){
      const {gutterX:gx1,gutterY:gy1}=this._bounds(this.scale);
      let nx=contentX*this.scale-(cx-rect.left)+gx1;
      let ny=contentY*this.scale-(cy-rect.top )+gy1;
      const c=this._clampXY(nx,ny,this.scale);
      this.scroll.scrollLeft=c.x; this.scroll.scrollTop=c.y;
      this._centerIfSmaller(this.scale);
    }
  }
  _onWheel(e){
  ...
  _pd(e){
    ...
    if(this._pts.size===2){
      ...
      this._cx=(a.clientX+b.clientX)/2; this._cy=(a.clientY+b.clientY)/2;
      this.scroll.style.touchAction='none';
    }
  }
  _pm(e){
    ...
    if(this._pinching&&this._pts.size===2){
      ...
      this._lastDist=d; e.preventDefault();
    }
  }
  _pu(e){
    this._pts.delete(e.pointerId);
    if(this._pts.size<2){
      this._pinching=false; this._lastDist=0; this.scroll.style.touchAction='pan-y';
      const c=this._clampXY(this.scroll.scrollLeft,this.scroll.scrollTop,this.scale);
      this.scroll.scrollLeft=c.x; this.scroll.scrollTop=c.y;
      this._centerIfSmaller(this.scale);
    }
  }
}
const zoom=new Zoom($('#pdf-container'),$('#pdf-scroll'));

/* Block page-level pinch inside our stage */
...
function centerHorizontally(){
  const baseW = zoom.baseW || CURRENT_PDF.vpCSSByPage[0]?.width ||
    parseFloat(CURRENT_PDF.wraps[0]?.querySelector('canvas')?.style.width||'0');
  if(!baseW) return;
  refs.scroll.scrollLeft=Math.max(0,(baseW*zoom.scale-refs.scroll.clientWidth)/2);
  const totalH=(zoom.baseH||refs.container.scrollHeight)*zoom.scale;
  ...
  attachOverlays(infos){
    ...
    infos.forEach(({wrap})=>{
      ...
      on(ov,'dblclick',e=>{
        ...
        if(this.mode==='text'){ const {x,y}=this._pos(ov,e); this._addText(ov,x,y,true); Session.bumpDirty?.(); }
...
const sigCtl=new SigController(sig,ann); refs.sigModal?.classList.remove('show');
...
async function renderPdfFromData(bytes,container,scale=1){
  ...
  // After DOM paints, capture stable base dimensions for zoom math
  await new Promise(r=>requestAnimationFrame(()=>{ zoom.setBaseSize(refs.container.scrollWidth, refs.container.scrollHeight); r(); }));
  // Build overlays and center
  const infos=CURRENT_PDF.wraps.map((wrap,i)=>({wrap,vpCSS:CURRENT_PDF.vpCSSByPage[i]}));
...
/* ---------- Text Settings (kept) ---------- */
const Settings=(()=>{let panel,sizeInp,colorInp,boldInp,italicInp,familySel;const state={size:16,color:'#000',bold:false,italic:false,family:'Arial, sans-serif'};
  function build(){panel=document.createElement('div');Object.assign(panel.style,{position:'fixed',right:'12px',bottom:'calc(88px + env(safe-area-inset-bottom,0px))',zIndex:'60',background:'rgba(20,24,32,.94)',border:'1px solid rgba(255,255,255,.14)',borderRadius:'12px',padding:'10px',minWidth:'240px',display:'none',color:'#e9eef5',font:'13px system-ui,sans-serif'});
    panel.innerHTML=`<div style="display:flex;gap:8px;align-items:center;margin-bottom:8px"><label style="display:flex;gap:6px;align-items:center">Size
      <input id="ts-size" type="range" min="16" max="48" step="1" value="${state.size}" />
      <span id="ts-size-val" style="width:28px;text-align:right">${state.size}</span></label></div>
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px">
      <label style="display:flex;gap:6px;align-items:center">Color <input id="ts-color" type="color" value="${state.color}" /></label>
      <label style="display:flex;gap:6px;align-items:center">Bold <input id="ts-bold" type="checkbox" ${state.bold?'checked':''} /></label>
      <label style="display:flex;gap:6px;align-items:center">Italic <input id="ts-italic" type="checkbox" ${state.italic?'checked':''} /></label></div>
      <div style="display:flex;gap:8px;align-items:center"><label style="display:flex;gap:6px;align-items:center">Font
      <select id="ts-family"><option>Arial, sans-serif</option><option>Helvetica, Arial, sans-serif</option><option>Georgia, serif</option><option>Times New Roman, Times, serif</option><option>Courier New, Courier, monospace</option><option>Inter, system-ui, sans-serif</option></select></label></div>`;
    document.body.appendChild(panel);
    sizeInp=panel.querySelector('#ts-size');colorInp=panel.querySelector('#ts-color');boldInp=panel.querySelector('#ts-bold');italicInp=panel.querySelector('#ts-italic');familySel=panel.querySelector('#ts-family');
    const apply=part=>{Object.assign(state,part);ann.textStyle={...ann.textStyle,...state};const el=ann.selected;if(el?.classList.contains('text')){el.style.fontSize=`${Math.max(16,state.size)}px`;el.style.color=state.color;el.style.fontWeight=state.bold?'700':'400';el.style.fontStyle=state.italic?'italic':'normal';el.style.fontFamily=state.family;Session.bumpDirty?.();}};
  ...
  function syncFromElement(el){const cs=getComputedStyle(el);const rgbToHex=rgb=>{const m=rgb.match(/\d+/g);if(!m)return'#000000';const[r,g,b]=m.map(n=>(+n)|0);return'#'+[r,g,b].map(v=>v.toString(16).padStart(2,'0')).join('');};
    ...
    Object.assign(state,next);panel.querySelector('#ts-size').value=String(state.size);panel.querySelector('#ts-size-val').textContent=state.size;panel.querySelector('#ts-color').value=state.color;panel.querySelector('#ts-bold').checked=state.bold;panel.querySelector('#ts-italic').checked=state.italic;panel.querySelector('#ts-family').value=state.family;ann.textStyle={...state};}
...
async function saveFlattened(){
  ...
  try{
    if(!isValidPdfBytes(CURRENT_PDF.bytes)){
      if(CURRENT_PDF.file){
        ...
        if(isValidPdfBytes(fresh)) CURRENT_PDF.bytes=fresh; else throw new Error('Read file but it is not a valid PDF');
      }else{ throw new Error('Invalid in-memory PDF buffer and no file handle'); }
    }
  }catch(rehydrateErr){ console.error('PDF buffer recovery failed:',rehydrateErr); toast('Could not read the original PDF. Please re-open the file and try again.','err',3800); return; }
  ...
  try{
    ...
    const blob=new Blob([out],{type:'application/pdf'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=defaultName;
    document.body.appendChild(a); a.click(); a.remove(); setTimeout(()=>URL.revokeObjectURL(a.href),1200); toast('Downloaded ✔️');
...
function wireUI(){
  ...
  on(refs.fileInput,'change',async e=>{
    ...
    catch(err){ const msg=String(err?.message||err); if(msg.includes('Worker')||msg.includes('loading')) toast('PDF engine failed to start. Check worker path.','err',4200); else toast('Could not open PDF','err',3200); console.error(err); return; }
  ...
  on(refs.signBtn,'click',()=>{ sigCtl.open(); }); // always open modal
