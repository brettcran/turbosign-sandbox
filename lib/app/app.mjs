/* TurboSign 4.0 — ESM, stable base
   - Inline SVG icons only (no external icon libs)
   - PDF.js v5 (ESM) + module worker from CDN
   - pdf-lib (ESM via +esm)
   - Text (dbl‑tap/click), Stamp ✓, Signature (draw → place), drag/resize, undo/redo
   - Centered pinch zoom / Ctrl+wheel zoom
   - Save (flatten) with pdf-lib
   - Minimal session restore (banner position controlled by CSS in index.html)
*/
...
try {
  ...
  console.warn('[PDF.js] Worker failed, falling back to main thread:', e);
...
(function wirePinch(){
  ...
  // Block iOS gesture default inside stage
  ...
  // Prevent browser pinch scrolling when 2+ touches
...
async function renderPdfFromData(bytes, container, scale=1){
  ...
  // after render, if snapshot belongs to this file, prompt restore
  tryPromptRestore();
}
...
function wireUI(){
  ...
  // Signature button always opens modal (to draw/new)
...
/* Quiet benign promise rejections (render aborts, etc.) */
