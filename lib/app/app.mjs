/* TurboSign — v4.1 Gold Stable
   - PDF.js v5 ESM core + module worker (CDN/local) with main-thread fallback
   - pdf-lib ESM for flatten/save
   - Text (dbl‑tap/click), ✓ stamp (black), signatures (modal → Use → dbl‑tap to place)
   - Drag text/sign; resize signatures; undo/redo; session restore; toasts
   - Centered pinch zoom (damped); ctrl/cmd+wheel zoom; Safari pinch guards
*/
...
try {
  // prefer local (if you host /lib/build/pdf.mjs) else CDN
  ...
  console.error('[PDF.js] Core load failed', e);
  throw e;
}

// Try module worker (.mjs); fallback to main-thread
async function ensureWorkerV5(){
  ...
  console.warn('[PDF.js] No worker available — running on main thread');
}
...
/* ---------- zoom controller (centered, damped) ---------- */
const zoom = {
  scale: 1,
  min: 0.6,
  max: 3.0,
  ...
  setBaseFromLayout(){
    ...
    h += 20 + 140; // container paddings
    this.baseW=Math.max(1,w); this.baseH=Math.max(1,h);
  },
  computeFitScale(){
...
/* prevent iOS browser pinch inside stage */
...
/* keep horizontally centered when size changes */
...
class Annotations {
  ...
  attachOverlays(wrapInfos){
    ...
    wrapInfos.forEach(({wrap})=>{
      ...
      // clear when clicking background
...
class SigPad{
  ...
  clear(){
    ...
    // white pad background, but export keeps only ink (transparent bg) when embedded as PNG
    c.fillStyle='#fff'; c.globalCompositeOperation='destination-over'; c.fillRect(0,0,this.canvas.width,this.canvas.height);
    c.globalCompositeOperation='source-over';
  }
  dataURL(){ return this.canvas.toDataURL('image/png'); } // transparent ink is preserved upon embed
}
