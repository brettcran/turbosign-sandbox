/* lib/app/app.mjs — clean build with signature clamp/scroll/flash */

/* ---------- PDF.js core & worker (ultra-robust) ---------- */
let pdfjsLib;
try {
  // Prefer local build if present (GitHub Pages: case-sensitive)
...
async function bootPdfWorkerOnce(){
  ...
  try {
    ...
    if (await urlOk(localURL)) {
      try {
        const t = new Worker(localURL, { type:'module' }); t.terminate();
  ...
  // 2) CDN classic worker (most compatible across browsers)
  ...
  if (await urlOk(cdnModule)) {
    try {
      const t = new Worker(cdnModule, { type:'module' }); t.terminate();
...
// Flash style for new items (signature visibility)
(() => {
...
function scrollAnnoIntoView(el){
  ...
  if (!wrap) return;
  // Center the element within the scroll view
...
class Annotations {
  ...
  _addText(overlay,x,y,focus){
    ...
    this._wireAnno(el, overlay, {resizable:false}); // no text resize
  ...
  _addSignature(overlay,x,y,dataURL,widthHint){
    ...
    img.onload=()=>{
      ...
      // Clamp inside page after we know its size
...
/* ---------- Signature Pad (transparent PNG output) ---------- */
class SigPad{
  ...
  dataURL(){ return this.canvas.toDataURL('image/png'); } // transparent bg
}
...
class SigController{
  ...
  use(){
    // Page under viewport center
    let target=this.ann.overlays[0]||null;
    if(this.ann.overlays.length){
      const midX=refs.scroll.scrollLeft + refs.scroll.clientWidth/2;
      const midY=refs.scroll.scrollTop  + refs.scroll.clientHeight/2;
      target=this.ann.overlays.find(ov=>{
    ...
    if(!target){ this.close(); toast('No page to place signature','err'); return; }

    // Place near visual center of that page (overlay coords, unscaled)
    const W = target.clientWidth, H = target.clientHeight;
    const x = (W/2); const y = (H/2);

    const el = this.ann._addSignature(target, x, y, this.sig.dataURL(), W*0.5);
    this.ann._select(el);
    scrollAnnoIntoView(el);
    Session.bumpDirty?.();
    this.close();
    toast('Signature added ✔️');
  }
}
...
async function renderPdfFromFile(file, container, scale=1){
  CURRENT_PDF.file = file; // keep original for save
