/* TurboSign — v4.0 Gold (Reconstructed)
   - PDF.js v5 ESM + module worker (CDN/local) with main-thread fallback
   - pdf-lib ESM save/flatten
   - Text (dbl‑tap/click), ✓ stamp (black), signature (modal → Use → dbl‑tap to place)
   - Drag text/sign; resize signatures; undo/redo; session restore; toasts
   - Centered pinch zoom; Safari gesture blocks
*/
...
function rgbToHex(rgb){ const m=String(rgb).match(/\d+/g); if(!m) return '#000000'; const [r,g,b]=m.map(n=>+n); return '#'+[r,g,b].map(v=>v.toString(16).padStart(2,'0')).join(''); }
...
try {
  ...
  pdfjsLib = await (await headOK(localCore) ? import(localCore) : import(CDN_BASE+'pdf.mjs'));
} catch (e) {
  console.error('[PDF.js] Core load failed', e); throw e;
}
async function ensureWorkerV5(){
  try{
    const localW = new URL('../build/pdf.worker.mjs', import.meta.url).href;
    if (await headOK(localW)){ const t=new Worker(localW,{type:'module'}); t.terminate();
      pdfjsLib.GlobalWorkerOptions.workerPort=new Worker(localW,{type:'module'}); console.info('[PDF.js] Worker: local'); return; }
  }catch{}
  try{
    const cdnW = CDN_BASE+'pdf.worker.mjs';
    if (await headOK(cdnW)){ const t=new Worker(cdnW,{type:'module'}); t.terminate();
      pdfjsLib.GlobalWorkerOptions.workerPort=new Worker(cdnW,{type:'module'}); console.info('[PDF.js] Worker: CDN'); return; }
  }catch{}
  pdfjsLib.GlobalWorkerOptions.workerPort=null; console.warn('[PDF.js] No worker — main thread render');
}
...
/* ---------- zoom (centered) ---------- */
const zoom = {
  scale: 1, min: 0.6, max: 3.0, suspended:false, baseW:1, baseH:1, fit:1,
  setBaseFromLayout(){
    let w=0,h=0;
    CURRENT_PDF.vpCSSByPage.forEach((vp,i)=>{ w=Math.max(w,vp.width); h+=vp.height; if(i>0) h+=12; });
    h += 20 + 140; this.baseW=Math.max(1,w); this.baseH=Math.max(1,h);
  },
  computeFitScale(){
    ...
    this.min = this.fit*0.9;
    this.max = this.fit*2.2;
  },
  apply(){ refs.container.style.transformOrigin='0 0'; refs.container.style.transform=`scale(${this.scale})`; },
  setScale(newScale, cx, cy){
    newScale = clamp(newScale, this.min, this.max);
    if(newScale===this.scale) return;

    const scroll=refs.scroll, rect=scroll.getBoundingClientRect();
    const contentW0=this.baseW*this.scale, contentH0=this.baseH*this.scale;
    const contentW1=this.baseW*newScale,  contentH1=this.baseH*newScale;
    const gx0=Math.max(0,(scroll.clientWidth -contentW0)/2), gy0=Math.max(0,(scroll.clientHeight-contentH0)/2);
    const gx1=Math.max(0,(scroll.clientWidth -contentW1)/2), gy1=Math.max(0,(scroll.clientHeight-contentH1)/2);
...
/* Pinch + wheel */
(function wirePinch(){
  const pts=new Map(); let last=0,cx=0,cy=0,pinching=false;

  const down=e=>{ if(zoom.suspended) return; pts.set(e.pointerId,e);
    if(pts.size===2){ const [a,b]=[...pts.values()]; last=Math.hypot(a.clientX-b.clientX,a.clientY-b.clientY); cx=(a.clientX+b.clientX)/2; cy=(a.clientY+b.clientY)/2; pinching=true; refs.scroll.style.touchAction='none'; }};
  const move=e=>{ if(!pts.has(e.pointerId)) return; pts.set(e.pointerId,e);
    if(pinching && pts.size===2){ const [a,b]=[...pts.values()]; const d=Math.hypot(a.clientX-b.clientX,a.clientY-b.clientY); cx=(a.clientX+b.clientX)/2; cy=(a.clientY+b.clientY)/2;
      if(last){ const raw=d/last; const damp=Math.pow(raw,0.35); zoom.setScale(zoom.scale*damp,cx,cy); }
      last=d; e.preventDefault(); }};
  const up=e=>{ pts.delete(e.pointerId); if(pts.size<2){ pinching=false; last=0; refs.scroll.style.touchAction='pan-y'; }};
...
/* Block Safari page pinch in stage */
...
/* Center horizontally after resizes */
...
on(window,'resize',()=>{ zoom.computeFitScale(); if(zoom.scale<zoom.min){ zoom.scale=zoom.min; zoom.apply(); } centerHorizontally(); });

/* ---------- Annotations ---------- */
class Annotations {
  constructor(container){
    this.container=container; this.mode=null; this.overlays=[];
    this.selected=null; this.history=[]; this.redoStack=[];
  ...
  _addSignature(overlay,x,y,dataURL,widthHint){
    ...
    img.onload=()=>{
      ...
      const {w:ew,h:eh}=this._elSize(el);
  ...
  undo(){ const last=this.history.pop(); if(!last) return;
    if(last.el?.parentNode){ last.el.parentNode.removeChild(last.el); if(this.selected===last.el) this._select(null); this.redoStack.push(last); }
    Session.bumpDirty?.();
  }
  redo(){ const next=this.redoStack.pop(); if(!next) return;
    if(next.el){ next.overlay.appendChild(next.el); this.history.push(next); }
    Session.bumpDirty?.();
  }
...
class SigPad{
  ...
  clear(){
    ...
    // white pad underlay, ink exported as normal PNG (transparent around strokes when embedded)
    c.fillStyle='#fff'; c.globalCompositeOperation='destination-over'; c.fillRect(0,0,this.canvas.width,this.canvas.height);
  ...
  function build(){
    ...
    panel.innerHTML=`
      ...
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px">
        ...
        <label style="display:flex;gap:8px;align-items:center">Bold
          <input id="ts-bold" type="checkbox" ${state.bold?'checked':''} />
        </label>
        <label style="display:flex;gap:8px;align-items:center">Italic
    ...
    boldInp=panel.querySelector('#ts-bold'); italicInp=panel.querySelector('#ts-italic'); familySel=panel.querySelector('#ts-family');
...
async function renderPdfFromData(bytes, container, scale=1){
  if(!looksLikePdf(bytes)){ toast('That file doesn’t look like a PDF','err'); throw new Error('Invalid PDF'); }
  container.innerHTML=''; CURRENT_PDF.wraps=[]; CURRENT_PDF.vpCSSByPage=[];
  let pdf;
  try{ pdf = await pdfjsLib.getDocument({ data: bytes }).promise; }
  catch(err){ console.error('getDocument failed', err); toast('Could not open PDF','err',4200); throw err; }
  ...
  zoom.setBaseFromLayout(); zoom.computeFitScale(); zoom.scale=zoom.fit; zoom.apply(); centerHorizontally();
}
          ...
          embedOps.push((async ()=>{
            try{
              ...
              if (img.src.startsWith('data:')){
                const bin=atob((img.src.split(',')[1]||'')); b=new Uint8Array(bin.length); for(let i=0;i<b.length;i++) b[i]=bin.charCodeAt(i);
              } else {
                const res=await fetch(img.src,{mode:'cors'}); b=new Uint8Array(await res.arrayBuffer());
              }
              const png = await pdf.embedPng(b);
...
function wireUI(){
  ...
  on(refs.fileInput,'change',async e=>{
    ...
    try{
      ...
      if(snap && snap.filename===(file.name||CURRENT_PDF.filename)){
  ...
  on(refs.textBtn,'click',()=>{ const next = ann.mode==='text'?null:'text'; ann.setMode(next); toast(next ? (isTouch?'Text: double‑tap':'Text: double‑click') : 'Tool off'); });
  on(refs.stampBtn,'click',()=>{ const next = ann.mode==='stamp'?null:'stamp'; ann.setMode(next); toast(next ? 'Stamp: tap/click' : 'Tool off'); });
  ...
  on(refs.helpBtn,'click',()=>toast('📂 Open a PDF. Text: double‑tap/click; Stamp: tap; Signature: draw→Use, then double‑tap/click to place; Save to flatten.','ok',4800));
