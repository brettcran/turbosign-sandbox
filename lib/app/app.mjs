/* TurboSign — app.mjs (fit-to-page default, damped pinch, bounded zoom) */
...
const zoom = {
  scale: 1,
  min:  0.5,   // will be recalculated after render
  max:  3.0,   // will be recalculated after render
  fit:  1,     // computed fit-to-page
  suspended:false,
  baseW:1, baseH:1,
  ...
  setBaseFromLayout(){
    ...
    h += 20 + 140; // match #pdf-container padding
    this.baseW=Math.max(1,w);
    this.baseH=Math.max(1,h);
  },

  computeFitScale(){
    // Fit the FIRST page fully in viewport by height, and also respect width
    const first = CURRENT_PDF.vpCSSByPage[0]; if(!first){ this.fit=1; return; }
    const availW = refs.scroll.clientWidth  - 24;  // small gutter
    const availH = refs.scroll.clientHeight - 160; // keep toolbar area
    const sH = availH / first.height;
    const sW = availW / Math.max(...CURRENT_PDF.vpCSSByPage.map(v=>v.width));
    // choose the smaller so the page fully fits both dimensions
    this.fit = Math.max(0.3, Math.min(sH, sW));
    // clamp bounds relative to fit
    this.min = this.fit * 0.9;    // a touch smaller than fit
    this.max = this.fit * 2.2;    // ~2.2x zoom-in
  },
...
/* pointer/trackpad pinch with damping */
(function wirePinch(){
  ...
  const move = (e)=>{ if(!pts.has(e.pointerId)) return; pts.set(e.pointerId,e);
    if(pinching && pts.size===2){
      ...
      if(lastDist){
        ...
        // damp it so scale changes gently
        const damp = Math.pow(raw, 0.35); // 0.35 → gentler; tweak 0.3–0.5
  ...
  // Ctrl/⌘ + wheel zoom with small exponential step
  refs.scroll.addEventListener('wheel', (e)=>{ 
    if(!(e.ctrlKey||e.metaKey)) return;
    e.preventDefault();
    // deltaY > 0 → zoom out; < 0 → zoom in
    const step = Math.pow(1.0015, -e.deltaY); // gentle
    zoom.setScale(zoom.scale * step, e.clientX, e.clientY);
  }, {passive:false});
})();

/* center horizontally at current scale */
...
async function renderPdfFromData(bytes, container, scale=1){
  ...
  // compute base and fit scales, apply fit
