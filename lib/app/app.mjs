...
class Annotations {
  ...
  attachOverlays(wrapInfos){
    ...
    wrapInfos.forEach((info)=>{
      ...
      // STAMP: single tap/click (only empty space)
      on(ov,'pointerdown', (e)=>{
  ...
  _addSignature(overlay,x,y,dataURL,widthHint){
    ...
    el.style.left=`${x}px`; el.style.top=`${y}px`;
    const img = new Image(); img.draggable=false; img.src=dataURL;
  ...
  _wireAnno(el, overlay, {resizable}){
    ...
    // also allow starting drag when tapping the inner image (signatures)
    const img = el.querySelector('img');
    if (img) on(img, 'pointerdown', startDrag, {passive:false});
...
async function renderPdfFromData(bytes, container, scale=1){
  ...
  // NOTE: no fitToWidth() here — initial view is exactly 1:1
