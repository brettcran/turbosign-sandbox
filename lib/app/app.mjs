...
class Zoom {
  ...
  fitToWidth(vpCssWidth){
    ...
    const base = Math.max(1, vpCssWidth || 800);             // intrinsic CSS width, scale=1
...
class Annotations {
  ...
  // minimal serialize/deserialize (for session restore)
  serialize(){
...
async function renderPdfFromData(bytes, container, scale=1.5){
  ...
  try {
    ...
    // If core load fails, bubble to caller
    throw err;
  }
  ...
  // robust mobile centering (do twice to survive iOS layout passes)
  ...
  // shape expected by attachOverlays
