...
class SigPad{
  constructor(canvas, modal){
    this.canvas=canvas; this.modal=modal; this.ctx=canvas.getContext('2d'); this.clear();
    this.drawing=false;
...
// ===== Robust pdf-lib loader (CDN ESM only; 3 fallbacks) =====
        ...
        if (el.classList.contains('text')){
          ...
          const p = (async ()=>{
            try{
              ...
              const png = await pdf.embedPng(bytes); // alpha preserved
