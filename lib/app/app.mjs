// TurboSign v4.1 — ESM + IconPark (Iconify) + Robust Save
// -------------------------------------------------------
...
let originalBytes = null;      // Uint8Array from file open
let filename = null;
let activeTool = null;         // 'text' | 'stamp' | 'sign' | null
let signatureDataURL = null;   // data:image/png from sig pad
...
async function renderPdfFromData(bytes) {
  ...
  for (let i=1;i<=pdfDoc.numPages;i++){
    ...
    const vp = page.getViewport({ scale:1 }); // 1:1 CSS px
...
function armOverlays(){
  refs.container.querySelectorAll('.overlay').forEach(ov=>{
    ov.replaceWith(ov.cloneNode(true)); // clear listeners
  });
  refs.container.querySelectorAll('.overlay').forEach(ov=>{
...
/* ---------- Save (flatten) with recovery ---------- */
async function saveFlattened(){
  // 1) Ensure valid source bytes; rehydrate from pdf.js if needed
  let src = originalBytes;
  if (!isValidPdfBytes(src)) {
    try {
      const buf = await pdfDoc?.getData?.();
      if (buf && buf.length) src = new Uint8Array(buf);
    } catch {}
  }
  if (!isValidPdfBytes(src)) {
  ...
  let PDFDocument, rgb;
  try {
    const mod = await loadPdfLib();
    PDFDocument = mod.PDFDocument; rgb = mod.rgb;
  } catch (e) {
  ...
  // 3) Load source
  let pdf;
  try { pdf = await PDFDocument.load(src); }
  catch (e) { console.error('PDFDocument.load failed', e); toast('Source PDF is invalid or unreadable','err'); return; }
        ...
        if (el.classList.contains('text')){
          ...
          const p = (async ()=>{
            ...
            if (img.src.startsWith('data:')){
              ...
              bytes = new Uint8Array(bin.length); for (let i=0;i<bin.length;i++) bytes[i]=bin.charCodeAt(i);
            } else {
