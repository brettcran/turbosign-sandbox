/* TurboSign — app.mjs (pinch centered on midpoint, no drift)
   - PDF.js v5 (module worker): local /lib/build/pdf.worker.mjs → CDN → main-thread fallback
   - PDFs load 1:1 and horizontally centered
   - Pinch zoom: re-centered around pinch midpoint (no slide-left/right)
   - Signature button ALWAYS opens modal; double‑tap/click to place signature
   - Text (double‑tap/click), Stamp ✓ (black), draggable annos, undo/redo
...
async function ensureWorkerV5(){
  ...
  pdfjsLib.GlobalWorkerOptions.workerPort = null; // fallback main thread
  console.error('[PDF.js] No worker available — rendering on main thread');
}
...
/* ---------- centered pinch zoom (midpoint anchored) ---------- */
const zoom = {
  scale: 1,
  min: 0.6,
  max: 3,
  ...
  setScale(newScale, cx, cy){
    ...
    // content size at current/next scale
    ...
    // gutters (centering gaps) before/after
    ...
    // content coordinate of the screen point (cx,cy) relative to current gutters
...
(function wirePinch(){
  ...
  // iOS Safari built-in gesture events (just block default inside stage)
  ...
  // prevent browser pinch if 2+ touches inside our scroll
...
function wireUI(){
  ...
  // Signature button ALWAYS opens the modal to (re)draw
...
/* keep function here to be available to center after initial render */
function scrollAnnoIntoView(el){
  if (!el) return;
  const wrap = el.closest('.page-wrap'); if (!wrap) return;
  const rectC = $('#pdf-container').getBoundingClientRect();
