/* TurboSign v4.1 Stable (Gold)
   - PDF.js v5 ESM core + worker fallback
   - pdf-lib for flatten/save
   - Annotations: text, stamp ✓, signatures
   - Undo/redo, session restore, toasts
   - Pinch zoom (centered) + wheel zoom
*/

import * as pdfjsLib from "https://cdn.jsdelivr.net/npm/pdfjs-dist@5.4.4/build/pdf.mjs";
import { PDFDocument, rgb, StandardFonts } from "https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.esm.js";
...
async function initWorker() {
  try {
    ...
    console.error("[PDF.js] Worker failed, running on main thread", e);
...
/* ---------- Annotations (simplified for stable) ---------- */
class Annotations {
  constructor(container){ this.container=container; this.mode=null; this.overlays=[]; }
  ...
  _addText(ov,x,y){ const el=document.createElement('div'); el.className='anno text'; el.contentEditable='true'; el.style.left=x+'px'; el.style.top=y+'px'; ov.appendChild(el); }
  _addStamp(ov,x,y){ const el=document.createElement('div'); el.className='anno stamp'; el.style.left=x+'px'; el.style.top=y+'px'; el.innerHTML='<svg viewBox="0 0 24 24"><path d="M5 12l5 5L20 7"/></svg>'; ov.appendChild(el); }
  _addSign(ov,x,y){ const el=document.createElement('div'); el.className='anno sign'; el.style.left=x+'px'; el.style.top=y+'px'; const img=new Image(); img.src=LAST_SIG_DATAURL; img.style.width='160px'; el.appendChild(img); ov.appendChild(el); }
}
...
on($('[data-act="help"]'),'click',()=>toast('📂 Open a PDF → Add text (dblclick), stamp ✓ (click), or signature (draw+use). Save to flatten.'));