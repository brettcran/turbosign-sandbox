...
class Zoom {
  ...
  fitToWidth(){
    const first = this.container.querySelector('.page-wrap'); if (!first) return;
    const stageW = this.scroll.clientWidth - 24;
    const currentW = first.getBoundingClientRect().width || 1;
    const target = Math.max(this.min, Math.min(this.max, this.scale * (stageW/currentW)));
    this.setScale(target, this.scroll.clientWidth/2, 0, true);
    this.scroll.scrollLeft = Math.max(0, (this.container.getBoundingClientRect().width*this.scale - this.scroll.clientWidth)/2);
  }
...
class Annotations {
  ...
  attachOverlays(wrapInfos){
    ...
    wrapInfos.forEach((info)=>{
      const ov = document.createElement('div'); ov.className='overlay'; info.wrap.appendChild(ov);
      ...
      on(ov,'dblclick', (e)=>{
        ...
        Session.bumpDirty(); // record change
      });
      ...
      if (isTouch){
        ...
        on(ov,'pointerdown',(e)=>{
          ...
          const {x,y} = this._pos(ov, e);
          const dt = now - lastTap; const dx=Math.abs(x-lastX), dy=Math.abs(y-lastY);
  ...
  // serialize minimal (for session restore)
  serialize(){
...
class SigController {
  ...
  use(){
    const ov = ann.overlays[0]; // center of first visible page for simplicity
    let target = ov;
    if (ann.overlays.length){
      // pick overlay nearest viewport center
...
// === Session restore (autosave to localStorage; offer restore on load) ===
const Session = (() => {
...
function wireUI(){
  ...
  on(refs.fileInput, 'change', async (e)=>{
    ...
    try {
      ...
      // if a snapshot exists for this filename, offer inline restore
      const snap = Session.loadSnapshot();
      if (snap && snap.filename === CURRENT_PDF.filename){
  ...
  on(refs.helpBtn,  'click', ()=> toast('📂 Open a PDF. Text: double‑tap/click; Stamp: tap; Signature: draw → Use. Drag/resize elements; Save to flatten.', 'ok', 3800));
...
// On first load: if a snapshot exists but no PDF open yet, let the user know
