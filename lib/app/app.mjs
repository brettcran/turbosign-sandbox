/* TurboSign — app.mjs (centered pinch using spacer + stable base size)
   - PDF.js v5 ESM + worker (local → CDN → main-thread fallback)
   - Base content size captured after render; invisible spacer sets scroll bounds
   - Pinch zoom anchors to finger midpoint; no drift; recenters correctly
   - Double‑tap: text/sign; stamp (✓ black); drag; signature resize; undo/redo
...
/* ---------- spacer & zoom (base size + midpoint anchoring) ---------- */
// Invisible spacer defines scrollable area so scroll bounds match scaled content.
const spacer=document.createElement('div');
Object.assign(spacer.style,{position:'absolute', left:'0', top:'0', width:'0px', height:'0px', pointerEvents:'none', zIndex:'0'});
refs.scroll.appendChild(spacer);

// BASE = true, unscaled content size (CSS px). We do NOT re-measure after transform.
const BASE={ width:0, height:0 };

const zoom={
  scale:1, min:0.6, max:3, suspended:false,
  _syncSpacer(){ spacer.style.width=`${Math.max(1,Math.floor(BASE.width*this.scale))}px`; spacer.style.height=`${Math.max(1,Math.floor(BASE.height*this.scale))}px`; },
  setScale(newScale,cx,cy){
    ...
    this._syncSpacer(); // expand/collapse scroll area
...
(function wirePinch(){
  ...
  // iOS Safari native pinch → block within stage
...
// Single definition used by tools
function scrollAnnoIntoView(el){
  ...
  const targetLeft=clamp(elCX-refs.scroll.clientWidth/2,0,Math.max(0,BASE.width*zoom.scale - refs.scroll.clientWidth));
  const targetTop =clamp(elCY-refs.scroll.clientHeight/2,0,Math.max(0,BASE.height*zoom.scale- refs.scroll.clientHeight));
...
async function renderPdfFromData(bytes,container,scale=1){
  ...
  for(let i=1;i<=pdf.numPages;i++){
    try{
      ...
      // match your stage spacing: .page-wrap margin/gap ≈ 24px total vertical
      totalH+=vpCSS.height + 24;
    }catch(e){ console.warn('Page render warning:',e); }
  }

  // Capture base size and sync spacer to current scale
  BASE.width = maxW;
  BASE.height= totalH;
  zoom._syncSpacer();
