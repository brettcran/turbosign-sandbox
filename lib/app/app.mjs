...
class Zoom {
  ...
  _centerIfSmaller(){
    const { contentW, viewW } = this._bounds();
    // Horizontally center tiny content by zeroing scroll and letting layout center it
    if (contentW <= viewW) this.scroll.scrollLeft = 0;
  }

  /* ---- scale around a screen point (cx,cy) using the SCROLL rect ---- */
  setScale(newScale, cx, cy, keepCenter=true){
    newScale = Math.max(this.min, Math.min(this.max, newScale));
    if (newScale === this.scale) return;

    const rect = this.scroll.getBoundingClientRect(); // <— anchor to viewport, not container
  ...
  _onWheel(e){
    ...
    if (!(e.ctrlKey || e.metaKey)) return; // ctrl/cmd + wheel
    e.preventDefault();
  ...
  _pm(e){
    ...
    if (this._pinching && this._pts.size === 2){
      ...
      // Track LIVE midpoint so it stays under your fingers
      this._cx = (a.clientX + b.clientX)/2;
      this._cy = (a.clientY + b.clientY)/2;
