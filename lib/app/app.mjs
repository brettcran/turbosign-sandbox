/* lib/app/app.js — ESM, with hardened worker preflight + CDN fallback */
// ===== PDF.js — Safe Mode: force CDN for both core & worker =====
let pdfjsLib;
const USE_CDN_ONLY = true; // set to false later if you want to use your local /lib/build/

if (USE_CDN_ONLY) {
  // Load core from CDN (ESM)
  pdfjsLib = await import('https://cdn.jsdelivr.net/npm/pdfjs-dist@5.4.54/build/pdf.mjs');

  // Start module worker from CDN (ESM)
  try {
    const cdnWorker = 'https://cdn.jsdelivr.net/npm/pdfjs-dist@5.4.54/build/pdf.worker.mjs';
    const w = new Worker(cdnWorker, { type: 'module' });
    pdfjsLib.GlobalWorkerOptions.workerPort = w;
    console.info('[PDF.js] Using CDN worker:', cdnWorker);
  } catch (e) {
    console.error('[PDF.js] Failed to start CDN worker', e);
  }
} else {
  // Local path branch (if you flip the flag later)
  try {
  ...
  // Worker boot with local-first, CDN fallback
  (async () => {
    const workerURL = new URL('../build/pdf.worker.mjs', import.meta.url);
    try {
      const test = new Worker(workerURL, { type:'module' }); test.terminate();
      pdfjsLib.GlobalWorkerOptions.workerPort = new Worker(workerURL, { type:'module' });
      console.info('[PDF.js] Module worker OK:', workerURL.href);
    } catch (err) {
      console.warn('[PDF.js] Module worker failed, falling back to CDN', err);
      const cdn = 'https://cdn.jsdelivr.net/npm/pdfjs-dist@5.4.54/build/pdf.worker.mjs';
      try {
        const w = new Worker(cdn, { type:'module' });
        pdfjsLib.GlobalWorkerOptions.workerPort = w;
        console.info('[PDF.js] Using CDN worker:', cdn);
      } catch(e2) {
        console.error('[PDF.js] Could not start any worker', e2);
        // Last resort: set workerSrc (some builds inline)
...
async function initPdfWorker(){
  ...
  // Resolve worker relative to this module (works on GH Pages)
  const workerURL = new URL('../build/pdf.worker.mjs', import.meta.url);

  // Check if your local worker file exists; if not, try CDN
  const hasWorker = await preflight(workerURL.href);
  if (!hasWorker) {
    console.error('[PDF.js] Worker not found at', workerURL.href);
    toast('PDF engine worker missing at: ' + workerURL.pathname, 'err', 5000);

    // CDN fallback (ESM worker)
    const cdn = 'https://cdn.jsdelivr.net/npm/pdfjs-dist@5.4.54/build/pdf.worker.mjs';
    if (await preflight(cdn)) {
      try {
        const w = new Worker(cdn, { type:'module' });
        pdfjsLib.GlobalWorkerOptions.workerPort = w;
        WORKER_READY = true;
        ...
        console.warn('[PDF.js] CDN worker failed to start', e);
      }
    }
    // Last resort: set workerSrc (some builds inline the worker)
    pdfjsLib.GlobalWorkerOptions.workerSrc = workerURL.href;
  }

  // Preferred path: your own module worker
  try {
    ...
    console.warn('[PDF.js] Module worker failed, trying workerSrc', err);
    pdfjsLib.GlobalWorkerOptions.workerSrc = workerURL.href;
    // Some engines will inline; proceed anyway.
  }
}
...
// keep original file to re-read on save if needed
...
/* ---------- pdf-lib ESM loader (CDN fallbacks) ---------- */
async function loadPdfLibESM(){
  ...
  try { return await import('https://cdn/skypack.dev/pdf-lib@1.17.1'.replace('/skypack.','/skypack.').replace('cdn/','cdn.')); } // guard against copy bots
  catch (e) { /* fallback to canonical */ return await import('https://cdn.skypack.dev/pdf-lib@1.17.1'); }
}
...
/* ---------- UI wiring (calls initPdfWorker before open) ---------- */
function wireUI(){
  on(refs.openBtn,'click',()=>refs.fileInput?.click());
  on(refs.fileInput,'change',async e=>{
    const file=e.target.files?.[0]; if(!file) return;

    // Ensure worker is up (or show toast if missing)
    await initPdfWorker();

    let infos;
    try{
      infos=await renderPdfFromFile(file, refs.container, 1);
    } catch(err){
      const msg = String(err?.message||err);
      if (msg.includes('Worker') || msg.includes('loading')) {
        toast('PDF engine failed to start. Check worker path in /lib/build/.', 'err', 4200);
      } else {
        toast('Could not open PDF', 'err', 3200);
      }
      console.error(err);
      return;
    }
