// TurboSign v4.3 — Sleek icons (Phosphor), centered zoom, exclusive tools, text settings,
// and a Scan/Photo modal with camera capture & file browse. Photos are draggable/resizable.
// -----------------------------------------------------------------------------------------
...
const refs = {
  ...
  // photo modal
  pm: $('#photo-modal'),
  pmUseCamera: $('#pm-use-camera'),
  pmChoosePhoto: $('#pm-choose-photo'),
  pmClose: $('#pm-close'),
  pmCameraSec: $('#pm-camera'),
  pmFileSec: $('#pm-file'),
  cameraView: $('#camera-view'),
  captureCanvas: $('#capture-canvas'),
  pmCapture: $('#pm-capture'),
  pmRetake: $('#pm-retake'),
  pmUsePhoto: $('#pm-use-photo'),
  pmBrowse: $('#pm-browse'),
};
...
const zoom = {
  ...
  setScale(newScale, cx, cy){
    ...
    scroll.scrollLeft = Math.max(0, Math.min(newScrollLeft, maxX));
    scroll.scrollTop  = Math.max(0, Math.min(newScrollTop , maxY));
...
(function wirePinch(){
  ...
  // Prevent browser pinch inside stage
...
async function renderPdfFromData(bytes) {
  ...
  for (let i=1;i<=pdfDoc.numPages;i++){
    ...
    const vp = page.getViewport({ scale:1 }); // 1:1 CSS
...
function centerHorizontally(){
  const w = refs.container.scrollWidth * zoom.scale;
  const desired = Math.max(0, (w - refs.scroll.clientWidth)/2);
...
function applyTextPanel(part){
  ...
  if (sel && sel.classList?.contains('text')) {
    sel.style.fontSize = `${Math.max(16,textStyle.size)}px`;
    sel.style.color = textStyle.color;
    sel.style.fontWeight = textStyle.bold ? '700':'400';
    sel.style.fontStyle = textStyle.italic ? 'italic':'normal';
    sel.style.fontFamily = textStyle.family;
  }
}
refs.tsSize.addEventListener('input', ()=>{
  refs.tsSizeVal.textContent = refs.tsSize.value;
  applyTextPanel({ size: +refs.tsSize.value });
},{passive:true});
...
/* ---------- Toolbar (exclusive tools) ---------- */
function setActiveTool(tool){
  activeTool = tool;
  document.querySelectorAll('#toolbar .btn').forEach(b=>{
    if (['text','stamp','sign'].includes(b.dataset.act)){
      const on = (b.dataset.act === tool);
      b.classList.toggle('active', on);
      // icon swap handled in index hover script; force filled when active
      const icon = b.querySelector('iconify-icon');
      const base = b.dataset.icon;
      icon?.setAttribute('icon', `ph:${on ? base+'-fill' : base.replace(/-fill$/,'')}`);
    } else {
...
refs.toolbar.addEventListener('click', e=>{
  ...
  if (act==='help'){ toast('Open a PDF → double‑tap to add Text/Stamp/Signature, or Scan/Insert Photo → Save to flatten.'); return; }
...
refs.fileInput.addEventListener('change', async e=>{
  ...
  try { await renderPdfFromData(originalBytes); }
...
const PhotoModal = (()=> {
  ...
  function open(){
    refs.pm.classList.add('show');
    // default view
    showSection('file');
    refs.pmRetake.style.display='none';
    refs.pmUsePhoto.style.display='none';
  }
  function close(){
    refs.pm.classList.remove('show');
    stopCamera();
  }
  async function startCamera(){
    try{
      ...
      refs.pmRetake.style.display='none';
      refs.pmUsePhoto.style.display='none';
      refs.pmCapture.style.display='';
    }catch(e){
      console.warn('Camera unavailable', e);
      toast('Camera unavailable. Choose a photo instead.', 'err', 3200);
      showSection('file');
    }
  }
  function stopCamera(){
  ...
  function captureFrame(){
    ...
    refs.pmCapture.style.display='none';
    refs.pmRetake.style.display='';
    refs.pmUsePhoto.style.display='';
  }
  function retake(){
    refs.pmCapture.style.display='';
    refs.pmRetake.style.display='none';
  ...
  function useCaptured(){
    ...
    placeImageOnFirstVisible(dataURL);
    close();
  }
  function choosePhoto(){
    refs.photoFile.value=''; refs.photoFile.click();
  }
  // place image at center of first visible page
  function placeImageOnFirstVisible(dataURL){
    const page = document.elementFromPoint(
      window.innerWidth/2,
      Math.min(window.innerHeight/2, refs.scroll.getBoundingClientRect().bottom - 10)
    )?.closest?.('.page-wrap') || refs.container.querySelector('.page-wrap');
    const ov = page?.querySelector?.('.overlay');
    if (!ov) { toast('Open a PDF first','err'); return; }
    const r = ov.getBoundingClientRect();
    const x = (r.width/2)/zoom.scale - 90, y = (r.height/2)/zoom.scale - 60;
    addImageLike(ov, x, y, dataURL, 'image');
    toast('Photo inserted ✔️');
  }

  // wire controls
  refs.pmUseCamera.addEventListener('click', startCamera);
  refs.pmChoosePhoto.addEventListener('click', ()=>{ stopCamera(); showSection('file'); });
  refs.pmClose.addEventListener('click', close);
  refs.pmCapture.addEventListener('click', captureFrame);
  ...
  // file chosen
  refs.photoFile.addEventListener('change', async e=>{
    const f = e.target.files?.[0]; if (!f) return;
    const reader = new FileReader();
    reader.onload = ()=>{ placeImageOnFirstVisible(String(reader.result||'')); close(); };
...
function isValidPdfBytes(bytes) {
  ...
  const maxSkip = Math.min(bytes.length, 16);
  for (let i=0; i<maxSkip-4; i++){
...
async function saveFlattened(){
  ...
  if (!isValidPdfBytes(bytes)) {
    try {
      ...
      if (data && data.length) bytes = new Uint8Array(data);
    } catch {}
  }
  if (!isValidPdfBytes(bytes)) {
    toast('Could not read the original PDF. Please reopen the file.', 'err', 3800);
    return;
  }

  let PDFDocument, rgb;
  try { ({ PDFDocument, rgb } = await loadPdfLib()); }
  catch(e){ console.error('pdf-lib load failed', e); toast('Could not load save engine','err'); return; }

  let pdf;
  try { pdf = await PDFDocument.load(bytes); }
  catch(e){ console.error('PDFDocument.load failed', e); toast('Source PDF is invalid','err'); return; }
  ...
  try {
    ...
    document.querySelectorAll('.page-wrap').forEach((wrap, idx)=>{
      ...
      wrap.querySelectorAll('.anno').forEach(el=>{
        ...
        if (el.classList.contains('text')){
          ...
          page.drawText(el.textContent || '', { x, y, size, color: rgb(color[0],color[1],color[2]) });
          ...
          const p = (async ()=>{
            ...
            if (img.src.startsWith('data:')){
              ...
              ib=new Uint8Array(bin.length); for(let i=0;i<ib.length;i++) ib[i]=bin.charCodeAt(i);
            } else {
              const res=await fetch(img.src,{mode:'cors'}); ib=new Uint8Array(await res.arrayBuffer());
            }
            const png = await pdf.embedPng(ib);
    ...
    console.error('Save failed', e); toast('Could not save PDF','err');
  }
}

/* ---------- File open handler ---------- */
document.addEventListener('DOMContentLoaded', ()=>{
  // nothing else needed here; toolbar icon swap is handled in index.html
});