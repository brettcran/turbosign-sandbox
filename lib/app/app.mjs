...
class Zoom {
  ...
  fitToWidth(vpCssWidth){
    ...
    const base = Math.max(1, vpCssWidth || 800);          // CSS width of a page at scale=1
...
// === Annotations (double‑tap/click to add text; drag/resize; dbl‑tap text to edit) ===
class Annotations {
  ...
  attachOverlays(wrapInfos){
    ...
    wrapInfos.forEach((info)=>{
      ...
      // STAMP: single tap/click (only when clicking empty space)
      on(ov,'pointerdown', (e)=>{
        if (this.mode !== 'stamp') return;
        if (e.target !== ov) return;                   // don't add over existing element
        const {x,y} = this._pos(ov, e);
  ...
  // --- coordinate helpers (all UN‑SCALED CSS px) ---
  _pos(overlay,e){                           // pointer → overlay-local coords (unscaled)
    const r = overlay.getBoundingClientRect();
    return { x:(e.clientX - r.left) / zoom.scale, y:(e.clientY - r.top) / zoom.scale };
  }
  _elSizeUnscaled(el){                       // current element size in unscaled px
    // offsetWidth/Height are layout (unscaled) sizes, works even when parent is transformed
  ...
  _onMove(e){
    if (this.drag.el){
      const {x,y} = this._pos(this.drag.overlay,e);          // unscaled
      let nx = x - this.drag.dx, ny = y - this.drag.dy;

      const overlayW = this.drag.overlay.clientWidth;
      const overlayH = this.drag.overlay.clientHeight;
    ...
    if (this.resize.el){
      // convert deltas from client (scaled) → unscaled CSS px
  ...
  // --- serialize/deserialize (unscaled CSS px)
  serialize(){
...
class SigController {
  ...
  use(){
    // overlay nearest viewport center
    let target = ann.overlays[0];
    ...
    const x = (rect.width / zoom.scale) * 0.5;  // place using unscaled coords
...
// === PDF rendering (robust; staged errors) ===
...
async function renderPdfFromData(bytes, container, scale=1.5){
  ...
  try {
    ...
    throw err; // fail early only if core load fails
  }
  ...
  for (let i=1;i<=pdf.numPages;i++){
    try {
      ...
      console.warn('Page render warning:', pageErr);
    }
  }

  // mobile centering (twice for iOS)
...
// === Save flattened PDF (uses UN‑SCALED CSS px) ===
async function saveFlattened(){
  ...
  CURRENT_PDF.wraps.forEach((wrap, idx)=>{
    ...
    overlay.querySelectorAll('.anno').forEach(el=>{
      const left = el.offsetLeft, top = el.offsetTop;            // unscaled px
      const w = el.offsetWidth,  h  = el.offsetHeight;           // unscaled px
      const x = left * fx;
...
function wireUI(){
  ...
  on(refs.fileInput, 'change', async (e)=>{
    ...
    // Stage 1: render (fatal on failure)
    let infos;
    try {
      infos = await renderPdfFromFile(file, refs.container, 1.5);
    } catch (renderErr) {
      console.error('PDF open failed:', renderErr);
      toast('Could not open PDF', 'err');
      return;
    }

    // Stage 2: overlays (non-fatal)
    try {
      ann.attachOverlays(infos); // infos shape: [{wrap, vpCSS}, ...]
    } catch (attachErr) {
      console.warn('Overlay attach issue:', attachErr);
    }

    toast('PDF loaded ✔️');

    // Stage 3: restore prompt (non-fatal)
    try {
      ...
      if (snap && snap.filename === (CURRENT_PDF.filename||file.name)){
        ...
        once(refs.restoreYes,'click', async ()=>{
          ...
          try {
            await ann.deserialize(snap.pages);
            toast('Session restored ✔️');
          } catch (dErr) {
            console.warn('Restore failed:', dErr);
            toast('Could not restore previous session', 'err');
          }
        });
      ...
      console.warn('Restore prompt issue:', restErr);
    }
  });
...
// On startup: show generic restore hint if a snapshot exists (no PDF open yet)
...
// Optional: suppress benign rejections in certain browsers
